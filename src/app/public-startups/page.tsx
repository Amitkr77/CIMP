...
type Startup = {
  _id: any; // or Types.ObjectId (better)
  startupName: string;
...
async function getPublicStartups(): Promise<Startup[]> {
  await connectDB();
  ...
  return (
    <main className="min-h-screen bg-bg">
      ...
      <nav
        ...
        style={{ boxShadow: "0 1px 3px rgba(15,33,68,0.06)" }}
      >
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link
            href="/dashboard"
            className="text-secondary hover:text-brand text-sm font-body transition-colors font-medium"
          >
            Dashboard →
          </Link>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-50 border border-green-200 mb-6">
            ...
            <span className="text-green-700 text-xs font-mono tracking-widest uppercase">
              CEO Approved
            </span>
          </div>
          ...
          <p className="text-secondary font-body text-lg max-w-xl mx-auto">
            Startups that earned their place through our rigorous review
            process.
          </p>
        </div>
        ...
        {serialized.length === 0 ? (
          <div className="text-center py-24 bg-white border border-border rounded-2xl shadow-card">
            ...
            <p className="text-muted font-body">
              No startups published yet. Check back soon.
            </p>
          </div>
        ) : (
          <>
            <p className="text-muted text-sm font-mono mb-8">
              {serialized.length} startup{serialized.length !== 1 ? "s" : ""}{" "}
              incubated
            </p>
              ...
              {serialized.map((s: any) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-4 flex-wrap">
                    <span className="text-xs font-mono text-green-700 bg-green-50 border border-green-200 px-2 py-0.5 rounded-full">
                      ✓ Incubated
                    </span>
                    {s.isFastTracked && (
                  ...
                  <h3 className="font-display text-2xl font-semibold text-navy mb-1 group-hover:text-brand transition-colors">
                    {s.startupName}
                  </h3>
                  <p className="text-muted text-xs font-body mb-3">
                    by {s.applicantName}
                  </p>
                  <p className="text-secondary text-sm font-body leading-relaxed flex-1 line-clamp-4">
                    {s.idea}
                  </p>
