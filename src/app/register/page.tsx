...
const ROLES = [
  { value: 'incubatee', label: 'Incubatee', desc: 'Apply with your startup idea', icon: '🚀' },
  { value: 'manager',   label: 'Manager',   desc: 'Review and evaluate applications', icon: '🔍' },
  { value: 'ceo',       label: 'CEO',        desc: 'Final approval & full system control', icon: '👑' },
];

export default function RegisterPage() {
  const router = useRouter();
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'incubatee' });
      ...
      <div className="relative z-10 w-full max-w-lg">
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2.5">
            ...
            <span className="font-display text-2xl font-semibold text-navy">Launchpad</span>
          </Link>
          <p className="text-muted text-sm mt-3 font-body">Create your account</p>
        </div>

        <div className="bg-white border border-border rounded-2xl p-8" style={{ boxShadow: '0 4px 24px rgba(15,33,68,0.08)' }}>
          <form onSubmit={handleSubmit} className="space-y-5">
            {/* Role selector */}
            <div>
              <label className="block text-sm font-body font-medium text-navy mb-2">I am a...</label>
              <div className="grid grid-cols-3 gap-2">
                {ROLES.map(r => (
                  <button key={r.value} type="button" onClick={() => setForm(p => ({ ...p, role: r.value }))}
                    ...
                    <div className={`text-xs font-body font-semibold ${form.role === r.value ? 'text-brand' : 'text-secondary'}`}>{r.label}</div>
                  </button>
                ))}
              </div>
              <p className="text-xs text-muted mt-2 font-body">{ROLES.find(r => r.value === form.role)?.desc}</p>
            </div>

            {[
              { key: 'name', label: 'Full Name', type: 'text', ph: 'Ratan Tata' },
              { key: 'email', label: 'Email', type: 'email', ph: 'you@example.com' },
              { key: 'password', label: 'Password', type: 'password', ph: 'Min 6 characters' },
            ].map(f => (
              <div key={f.key}>
                <label className="block text-sm font-body font-medium text-navy mb-1.5">{f.label}</label>
        ...
        <p className="text-center text-muted text-sm mt-5 font-body">
          ...
          <Link href="/login" className="text-brand hover:text-navy font-medium transition-colors">Sign in</Link>
        </p>
      </div>
    </main>
  );
